module Textor
{
    export class LanguageService
    {
        ...
        {
            var timeout: number = new Date().getTime() + 20;

            var startPosition: TextPosition = new TextPosition(this._line, this._column);
            var line: number = this._textReader.textPosition.line;
            var column: number = this._textReader.textPosition.column;

            while (this._textReader.peek().length > 0)
            {
                var data: ILanguageStyle = this._language.read();
        ...
        public log()
        {
            for (var line: number = 0; line < this._syntaxTable.length; line++)
            {
                var text: string = "line " + line + ": ";
                if (this._syntaxTable[line])
                {
                    text += "[ ";
                    for (var i: number = 0; i < this._syntaxTable[line].length; i++)
                    {
