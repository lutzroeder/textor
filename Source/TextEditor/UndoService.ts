    ...
    export class UndoService
    {
        ...
        public commit()
        {
            if (!this._container.isEmpty)
            {
                ...
                var c1: ContainerUndoUnit = this._stack[this._stack.length - 1];
                ...
                if (this._stack.length > 1)
                {
                    ...
                    var c2: ContainerUndoUnit = this._stack[this._stack.length - 2];
        ...
        {
            var text: string = "";
            for (var i: number = 0; i < this._stack.length; i++)
            {
